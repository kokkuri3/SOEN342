...
class Instructor implements Serializable {
    ...
    public static void saveInstructors(List<Instructor> instructors, String fileName) throws IOException {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(fileName))) {
            oos.writeObject(instructors);
        }
    }

    public static List<Instructor> loadInstructors(String fileName) throws IOException, ClassNotFoundException {
...
class Offering {
    ...
    public static void saveOfferings(List<Offering> offerings, String fileName) throws IOException {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(fileName))) {
            oos.writeObject(offerings);
        }
    }

    public static List<Offering> loadOfferings(String fileName) throws IOException, ClassNotFoundException {
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(fileName))) {
            return (List<Offering>) ois.readObject();
        }
    }
...
class ProcessOfferings {
    ...
    public void saveData() {
        try {
            Instructor.saveInstructors(instructors, "instructors.dat");
            Offering.saveOfferings(offerings, "offerings.dat");
    ...
    public void loadData() {
        try {
            instructors = Instructor.loadInstructors("instructors.dat");
            offerings = Offering.loadOfferings("offerings.dat");
...
public class Main {
    public static void main(String[] args) {
        ...
        // Add some test data
        processOfferings.registerInstructor("John Doe", "123-456-7890", "Yoga", List.of("New York", "Los Angeles"));
        processOfferings.addOffering("Yoga", 10, "New York", new Date(), new Date());

        // Save data
        processOfferings.saveData();

        // Load data
        processOfferings.loadData();
    }
}
